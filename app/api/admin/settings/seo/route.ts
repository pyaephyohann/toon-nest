 ...
 * SEO settings endpoints (admin only)
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    return successResponse(settings, "SEO settings retrieved successfully");
  } catch (error) {
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const settings = await settingsService.updateSEOSettings(body);
