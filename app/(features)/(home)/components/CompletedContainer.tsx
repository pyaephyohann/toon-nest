...
export default function CompletedContainer({ title = "Completed Series" }: Props) {
    ...
    return (
      <section className="space-y-4">
        ...
        <div className="flex min-h-[200px] items-center justify-center rounded-2xl border border-border bg-card p-8">
          <p className="text-muted-foreground">Failed to load completed series.</p>
        </div>
      </section>
    );
  }
  ...
  if (!data || data.items.length === 0) {
    return (
      <section className="space-y-4">
        <h2 className="text-2xl font-bold">{title}</h2>
        <div className="flex min-h-[200px] items-center justify-center rounded-2xl border border-border bg-card p-8">
          <p className="text-muted-foreground">No completed series available.</p>
        </div>
      </section>
    );
  }
